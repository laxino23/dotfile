...
-- set remember to the cursor position and fold next time run the file
...
vim.api.nvim_create_autocmd("BufReadPost", {
  ...
  callback = function()
    ...
    -- Wait 200ms for nvim-ufo to finish initializing, then load the view
    vim.defer_fn(function()
      vim.cmd("silent! loadview")
      -- Optional: Open at least one level if everything is closed
      -- vim.cmd("normal! zx")
    end, 200)
  end,
})
