...
M.is_lsp_compatible_with_ft = function(server_name, ft)
	...
	if server_name == "efm" then
		if _G.efm_configs and _G.efm_configs[ft] then
			return true
		end
...
M.ensure_lsp_for_buffer = function(server_name, bufnr)
	...
	-- Define the correct prefixes
	local core_prefix = "languages.lsp.core.lsp." -- Correct path
	local extra_prefix = "languages.lsp.extra.lsp." -- Correct path

	if server_name == "efm" then
		-- Try extra (user) config first
		ok, mod = pcall(require, extra_prefix .. "efm")
		if not ok or type(mod) ~= "table" or not mod.config then
			-- Fallback to core config
			ok, mod = pcall(require, core_prefix .. "efm")
		end
	else
		-- Try extra (user) config first
		ok, mod = pcall(require, extra_prefix .. server_name)
		if not ok or type(mod) ~= "table" or not mod.config then
			-- Fallback to core config
			ok, mod = pcall(require, core_prefix .. server_name)
		end
	end

	if not ok or type(mod) ~= "table" or not mod.config then
		vim.notify("Config not found for: " .. server_name, vim.log.levels.WARN)
		return nil
	end
...
M.stop_servers_for_old_project = function()
	...
	for _, client in ipairs(clients) do
		if client.config and client.config.root_dir then
			...
			if
				type(client.config.root_dir) ~= "function"
				and client_root ~= current_dir
				and not vim.startswith(client_root, current_dir)
			then
				local cid = client.id
